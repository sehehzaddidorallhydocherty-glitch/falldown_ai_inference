...
static std::atomic<bool> s_server_shutting_down_{false};

void handle_message (k_s32 s32Id, k_ipcmsg_message_t* msg) {
    if (s_server_shutting_down_.load()) {
        return;
    }
...
int UserCommServer::Impl::Init(const std::string &service_name, IServerCallback *callback, int port) {
    ...
    s_server_shutting_down_.store(false);
    ...
    if (ret != K_SUCCESS) {
        std::cout << "UserCommServer::Impl::Init() add service failed, try cleanup stale service" << std::endl;
        kd_ipcmsg_del_service(service_name.c_str());
        ret = kd_ipcmsg_add_service(service_name.c_str(), &stConnectAttr);
        if (ret != K_SUCCESS) {
            return -1;
        }
    }
...
void UserCommServer::Impl::DeInit() {
    ...
    s_server_shutting_down_.store(true);
    s_callback_ = nullptr;
    s_msg_callback_ = nullptr;
